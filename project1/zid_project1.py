    ...
    with open(pth, 'r') as file:
        for line in file:
            if line.strip():  # Check if the line is not empty
                ...
                exchange = exchange.strip().lower().replace('"', '')
                ticker = ticker.strip().lower().replace('"', '')
                if exchange.isalpha() and ticker.isalpha():
                    tic_exchange_dic[ticker] = exchange

    return tic_exchange_dic
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    # Construct the file path using DATDIR and the ticker symbol
    file_path = os.path.join(DATDIR, f'{tic.lower()}_prc.dat')

    # Check if the file exists
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"The data file for ticker '{tic}' does not exist at path: {file_path}")
...
def verify_cols(col_lst=None):
    ...
    """
    if col_lst is None or not col_lst:
        raise Exception("The provided column list is empty or None.")

    for col in col_lst:
        if col not in COLUMNS:
            raise Exception(f"Column '{col}' is not found in the predefined COLUMNS list.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # Loop through each ticker and collect its data
    for ticker in tickers_lst:
        ticker_data = read_dat(ticker)
        data_list = []
        for line in ticker_data:
            line_dict = line_to_dict(line)
            # Filter dictionary to only include specified columns
            filtered_dict = {col: line_dict[col] for col in col_lst}
            data_list.append(filtered_dict)

        # Create the data dictionary for the ticker
        data_dict[ticker] = {
            'exchange': tic_exchange_dic[ticker],  # tic_exchange
            'data': data_list  # List of dictionaries (dict_0 to dict_n)
        }
...
# ----------------------------------------------------------------------------
    """1. Configuring as in step 1 ensures that the module is portable and can run on any computer without modification.
          This approach combines the base project directory and sub directories correctly, regardless of the operating
          system. It avoids issues with different path formats (macOS vs Windows format) and different file structures.
          By keeping paths dynamic, the project remains organized and easier to share, maintain, and deploy across
          different environment.   
    
       2. Hypothesis 2 is more likely to be true. The explanation is as follows.
          
          Hypothesis 1 suggests that articles are based on investors' evaluations of firms. If this were true, negative 
          articles would likely reflect current investor sentiments, causing short-term stock price drops. However, as 
          market sentiments change, we might expect stock prices to recover over time. Since we observed that stock 
          returns decrease in the short run with no reversal in the long run, this contradicts Hypothesis 1.
          
          Hypothesis 2 proposes that articles provide valuable information beyond what is already known about the firms.
          The observed data supports this hypothesis: negative articles lead to a short-term drop in stock prices with 
          no recovery. This suggests that the articles are introducing new, significant information that impacts the 
          firm’s perceived value. Investors react to this new information by adjusting their expectations, resulting in 
          a lasting decrease in stock prices.
          
          In conclusion, Hypothesis 2 is more likely to be true. The lasting impact of negative articles on stock prices
          suggests that they contain valuable new information about the firm, beyond what investors already know.
          
       3. In hypothesis 2 situation, in the short-run, the trading volume is predicted to increase. 
          When articles provide valuable information beyond firm fundamental, it will directly and quickly affects 
          how investors view the firm.
          New and unexpected information usually causes higher trading volume as market participants adjust their 
          positions based on the updated information. In this case, the valuable insights provided by the articles 
          would lead to a lot of trading activity, as the market tries to reprice the firm's stock based on 
          the new data.
          Hypothesis 2 situation also provokes more speculative trading. Without waiting the periodic data from the 
          companies, based on recent news, traders may engage in speculative trades to capitalize on anticipated price 
          movements. 
